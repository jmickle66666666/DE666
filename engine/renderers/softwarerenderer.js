...
var SoftwareRenderer = { name: "SoftwareRenderer", ctx: null, canvas: null , imgData: null, font:{characters:[], width:0, height:0, img:null}};
...
SoftwareRenderer.init = function(width, height, targetElement) {
    ...
    //SoftwareRenderer.canvas.style.position = "relative";
    ...
    SoftwareRenderer.loadFont("engine/fontsmall.png");
}

SoftwareRenderer.loadFont = function(texturePath) {
    if (SoftwareRenderer.font.img != null) SoftwareRenderer.font.img.remove()
    let img = document.createElement("img");
    SoftwareRenderer.font.img = img;
    img.src = texturePath;
    ...
    function loadit() {
        ...
        var font = SoftwareRenderer.font;
...
SoftwareRenderer.unload = function () {
    SoftwareRenderer.canvas.remove();
    if (SoftwareRenderer.font.img != null) SoftwareRenderer.font.img.remove();
}
...
SoftwareRenderer.clear = function() {
    SoftwareRenderer.imgData.data = new Uint8ClampedArray(SoftwareRenderer.imgData.data.length);
}
    ...
    for (let j = -radius; j <= radius+1; j++) {
        for (let i = -radius; i <= radius+1; i++) {
            ...
            if ((filled && d < d2) || (!filled && Math.abs(d2 - d)<Math.sqrt(d)))  {
                ...
                SoftwareRenderer.imgData.data[index + 0] = color.red; 
                SoftwareRenderer.imgData.data[index + 1] = color.green; 
                SoftwareRenderer.imgData.data[index + 2] = color.blue; 
                SoftwareRenderer.imgData.data[index + 3] = color.opacity * 255; 
            }
        }
    }
...
SoftwareRenderer.line = function (x1, y1, x2, y2, color, colorEnd) {
    ...
    for (let a = a1; a != a2; a+=da) {
        ...
        SoftwareRenderer.imgData.data[index + 3] = c.opacity * 255; 
    }
}
...
SoftwareRenderer.text = function (text, x, y, color) {
    ...
    for (let i = 0; i < text.length; i++) {
        if (text[i] == "\n") {
            _x = x;
            _y += SoftwareRenderer.font.height;
            continue;
        }
        SoftwareRenderer.character(text[i], _x, _y, color);
        _x += SoftwareRenderer.font.width;
    }
}
    ...
    for (let j = 0; j < font.height; j++) {
        for (let i = 0; i < font.width; i++) {
            if (character[i + j * font.width]) {
                var index = (Math.floor(x+i) + Math.floor(y+j) * SoftwareRenderer.imgData.width) * 4;
                SoftwareRenderer.imgData.data [index + 0] = color.red;
                SoftwareRenderer.imgData.data [index + 1] = color.green;
                SoftwareRenderer.imgData.data [index + 2] = color.blue;
                SoftwareRenderer.imgData.data [index + 3] = color.opacity * 255;
...
SoftwareRenderer.texture = function (texturePath, x, y, rectangle, transformation) {
    ...
    let swidth = SoftwareRenderer.imgData.width
    ...
    for (let j = rectangle.y; j < rectangle.y + rectangle.h; j++) {
        for (let i = rectangle.x; i < rectangle.x + rectangle.w; i++) {
            ...
            function pixel (_x, _y) {
                ...
                SoftwareRenderer.imgData.data[t_index + 3] = 255;
            }
